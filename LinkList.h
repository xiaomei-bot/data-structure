...
//算法--有序表的合并
template<class DT>
void he(LNode<DT>*& L1, LNode<DT>*& L2)
{
	LNode<DT>* p, * q, * s;
	p = L1->next;
	q = L2->next;
...
//算法--交集
template<class DT>
void jiao(LNode<DT>*& L1, LNode<DT>*& L2)
{
	//LNode<DT>* p, * q;
	//p = L1->next;
	//q = L2->next;
...
//算法--并集
template<class DT>
void bing(LNode<DT>*& L1, LNode<DT>*& L2)
{
	//LNode<DT>* p, * q;
	//p = L1->next;
	//q = L2->next;
	...
	for (int i = 1; i <= ListLength(L1); i++)
	{
		...
		{
			InsertElem_i(L2, ListLength(L2) + 1, e);
		}
	}
	cout << "两链表的并集是：" << endl;
	DispList(L2);
